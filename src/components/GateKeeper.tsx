...
export default function GateKeeper({ onUnlock, onClose }: GateKeeperProps) {
  ...
  const [step, setStep] = useState<"idle" | "verifying" | "denied">("idle");
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (ADMIN_EMAIL && user.email?.toLowerCase() !== ADMIN_EMAIL.toLowerCase()) {
        await signOut(auth);
        setStep("denied");
        setErrorMsg(`Access denied. This portal is restricted to the site administrator. You signed in as: ${user.email}`);
        setLoading(false);
        return;
      }
      ...
      <main className="max-w-sm w-full mx-auto px-6 py-10 flex flex-col items-center">
        ...
        <div className="text-center mb-8">
          ...
          <p className="font-mono text-[9px] text-[#8b8780] mt-2 leading-relaxed uppercase tracking-wider">
            Restricted to the site curator. Sign in with your authorized Google account.
          </p>
        </div>

        {/* Eye / Loader centrepiece */}
        ...
        {/* Error message */}
        {errorMsg && (
          <div className={`w-full p-3 border text-center mb-4 ${step === "denied" ? "border-red-300 bg-red-50 text-red-700" : "border-amber-200 bg-amber-50 text-amber-800"}`}>
            <p className="font-mono text-[9px] tracking-wider uppercase leading-relaxed">{errorMsg}</p>
            {step === "denied" && (
              <button
                onClick={() => { setStep("idle"); setErrorMsg(""); }}
                className="mt-2 font-mono text-[8px] uppercase tracking-wider text-red-600 hover:text-red-900 underline cursor-pointer"
              >
